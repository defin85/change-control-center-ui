...
- Non-Goals:
  ...
  - Redefine backend ownership, delivery verification, or parity targets already covered by other changes.
...
2. Introduce a `platform` layer with canonical workspace, master-detail, detail, and authoring shells.
3. Move complex workflow surfaces onto explicit state models where transitions matter.
4. Restrict route-level and workspace-level UI composition so it cannot bypass the platform layer.
5. Add validation gates that detect platform-boundary violations and browser-level conformance regressions.
